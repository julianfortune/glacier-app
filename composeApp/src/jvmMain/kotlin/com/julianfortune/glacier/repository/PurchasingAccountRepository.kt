...
class PurchasingAccountRepository(private val database: Database) : NamedEntityRepository<PurchasingAccount> {
    ...
    override fun getAll(): Flow<List<PurchasingAccount>> {
        return database.purchasingAccountQueries.getAll()
            .asFlow()
            .mapToList(Dispatchers.IO)
            .map { accounts ->
                accounts.map { s ->
                    PurchasingAccount(s.id, s.name)
                }
            }
    }
...
//    }
//
//    override suspend fun update(entity: PurchasingAccount) {
//        database.purchasingAccountQueries.updateById(entity.name, entity.id)
//    }
//
